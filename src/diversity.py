...
def compute_pcoa(
    ...
    timing = timing or TimingRecorder(None, component="diversity")
    ids = feature_table.index.tolist()
    matrix = feature_table.to_numpy(dtype=float)
    with timing.step("beta_diversity", item=metric):
        dm = beta_diversity(metric=metric, counts=matrix, ids=ids)
    with timing.step("pcoa", item=metric):
        ord_res = pcoa(dm)
    return dm, ord_res
...
def run(args: argparse.Namespace, timing: TimingRecorder) -> int:
    ...
    with timing.step("load_feature_table", item=str(feature_fp)):
        feature_table = pd.read_csv(feature_fp, sep="\t", index_col=0)
    dm, ord_res = compute_pcoa(feature_table, metric=args.metric, timing=timing)
...
def main() -> int:
    args = parse_args()
    timing = TimingRecorder(args.timings_tsv, component="diversity")
